...
//            result = false;
//            message = "微信支付失败，微信参数有误，请联系滁苏科技技术人员";
...
//            result = false;
//            message = "微信支付失败，订单(ID:" + orderId + ")不存在，请联系滁苏科技技术人员";
//            return renderString(response, result, message, data);
//        }
//
...
//    }
    ...
    public String notify(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (resultCode.equalsIgnoreCase("SUCCESS")) {
            ...
            if (StringUtils.isBlank(orderId) || order == null) {
                myReturnCode = "FAIL";
                logger.info("微信支付失败，订单(ID:" + orderId + ")不存在，请联系滁苏科技技术人员");
            ...
            if (StringUtils.isBlank(cashFee) || !totalPrice.equals(order.getTotalPrice())) {
                myReturnCode = "FAIL";
                logger.info("微信支付失败，订单金额(订单ID:" + orderId + "，金额:" + totalPrice + ")不正确，请联系滁苏科技技术人员");
                return writeToWxpay(myReturnCode, myReturnErr, response);
            }
            if (StringUtils.isBlank(appid) || !appid.equals(ConfigUtil.APPID)) {
                myReturnCode = "FAIL";
                logger.info("微信支付失败，配置不正确(APPID:" + appid + ")，请联系滁苏科技技术人员");
                return writeToWxpay(myReturnCode, myReturnErr, response);
            }
            if (StringUtils.isBlank(mchId) || !mchId.equals(ConfigUtil.MCH_ID)) {
                myReturnCode = "FAIL";
                logger.info("微信支付失败，配置不正确(MCH_ID:" + mchId + ")，请联系滁苏科技技术人员");
                return writeToWxpay(myReturnCode, myReturnErr, response);
            }
            if (StringUtils.isBlank(appid) || !appid.equals(ConfigUtil.APPID)) {
                myReturnCode = "FAIL";
                logger.info("微信支付失败，配置不正确(APPID:" + orderId + ")，请联系滁苏科技技术人员");
                return writeToWxpay(myReturnCode, myReturnErr, response);
            }
