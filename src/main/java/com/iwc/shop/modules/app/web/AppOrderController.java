...
//            page.setPageNo(Integer.parseInt(cunton)*5);//当前页码
//            page.setPageSize(5);// 每页页显示
//            page.setOrderBy("a.create_date desc");
            ...
            order.setSqlMap(sqlMap);
                String cunton = request.getParameter("cunton");
                if(StringUtils.isBlank(cunton)){
                    cunton = "0";
                }
            order.setBeginIndex(Integer.parseInt(cunton)*CodeConstant.PAGE_ORDER);//当前页码
            order.setEndIndex(CodeConstant.PAGE_ORDER);// 每页页显示
//            page = orderService.findPage(page, order);
            List<Order> orders = orderService.findList_order(order);
            page.setList(orders);
    ...
    public String listPendingPay(HttpServletRequest request, HttpServletResponse response) {
        ...
        List<Order> orderList = orderService.findPendingPay(userId,order);
    ...
    public String listDelivering(HttpServletRequest request, HttpServletResponse response) {
        ...
        order.setBeginIndex(Integer.parseInt(cunton)*CodeConstant.PAGE_ORDER);//当前页码
        order.setEndIndex(CodeConstant.PAGE_ORDER);// 每页页显示
        List<Order> orderList = orderService.findDelivering(userId,order);
    ...
    public String listFinished(HttpServletRequest request, HttpServletResponse response) {
        ...
        order.setBeginIndex(Integer.parseInt(cunton)*CodeConstant.PAGE_ORDER);//当前页码
        order.setEndIndex(CodeConstant.PAGE_ORDER);// 每页页显示
        List<Order> orderList = orderService.findFinished(userId,order);
...
//        order.setPayType(preorder.getPayType());//
        ...
        order.setRoughPayType(Order.ROUGH_PAY_TYPE_CASH);// 在线支付 还是现金支付
...
//        }
	...
	public String success(@PathVariable String id,
        ...
        String successDeliveryText = "由滁苏科技专人配送";
