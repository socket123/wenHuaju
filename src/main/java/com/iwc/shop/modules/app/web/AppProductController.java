...
//    }
     ...
     * 查看 搜索 商品列表
     */
    @RequestMapping(value = "/list-featured-Search")
    public String findAppFeaturedSearch(HttpServletRequest request, HttpServletResponse response) {
        boolean result;
        String message;
        Map<String, Object> data = Maps.newHashMap();
        List<Map<String, Object>> oProductList = Lists.newArrayList();
        ...
        List<ShopProduct> list = productService.findFeaturedPriceNameSearch_list(name);
        for (ShopProduct product : list) {
            Map<String, Object> oProduct = product.toSimpleObj();
            oProductList.add(oProduct);
        }
     ...
     * 查看app 首页推荐 商品列表
     */
    @RequestMapping(value = "/list-featured-Home")
    public String findAppFeaturedHome(HttpServletResponse response) {
        boolean result;
        String message;
        Map<String, Object> data = Maps.newHashMap();
        List<Map<String, Object>> oProductList = Lists.newArrayList();
        List<ShopProduct> list = productService.findAppFeaturedHome(10);
        for (ShopProduct product : list) {
            Map<String, Object> oProduct = product.toSimpleObj();
            oProductList.add(oProduct);
        }
