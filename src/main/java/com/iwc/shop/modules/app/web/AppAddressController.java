...
public class AppAddressController extends AppBaseController {
	...
	public String add(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String areaId = request.getParameter("areaId");//行政区编号
		String telephone = request.getParameter("telephone");//电话
		String detail = request.getParameter("detail");// 详情地址
		String fullname = request.getParameter("fullname");// 名称
    ...
    public String getId(@PathVariable String id, HttpServletRequest request, HttpServletResponse response) {
        if (!isLoggedIn(request)) {
            return renderNotLoggedIn(response);
        }
        ...
        message = "";
        data.put("address", oAddress);
        return renderString(response, result, message, address);
    }
